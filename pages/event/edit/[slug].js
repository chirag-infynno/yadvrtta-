...
const EditEvent = () => {
  ...
  useEffect(() => {
    formData?.latitude && formData?.longitude && submitStatus === "PUBLISHED"
      ? handleChangeSchema()
      : setSchema(createEventsFormValidation);
  }, [submitStatus, formData?.latitude]);
      ...
      <Formik
        ...
        initialValues={{
          ...
          startDate: formData?.startDate
            ...
            : null,
          startTime: formData?.startDate ? moment(formData?.startDate) : null,
          endDate: formData?.endDate
            ? moment(formData?.endDate).toDate()
            : null,
          endTime: formData?.endDate ? moment(formData?.endDate) : null,
          coverImage: formData?.coverImage,
          shortDescription: formData?.shortDescription,
          description: formData?.description,
          termsConditions: formData?.termsConditions,
          tickets:
            formData?.EventTicket.map((ticket) =>
              ticket.action
                ? ticket
                : {
                    ...ticket,
                    startDate: ticket?.startDate
                      ? moment(ticket?.startDate).toDate()
                      : null,
                    startTime: ticket?.startDate
                      ? moment(ticket?.startDate)
                      : null,
                    endDate: ticket?.endDate
                      ? moment(ticket?.endDate).toDate()
                      : null,
                    endTime: ticket?.endDate ? moment(ticket?.endDate) : null,
                    action: "NOCHANGE",
                  }
            ) || [],
        ...
        validationSchema={
          submitStatus === "PUBLISHED" ? schema : eventsSaveAsDraftValidation
        }
        onSubmit={(values) => {
          // console.log(values);
      ...
      >
        {(formik) => (
          <Form onSubmit={formik.handleSubmit}>
            <div className="py-81 px-139 mx-auto max-w-2xl xxl:px-139 xl:px-139 lg:px-90 md:px-5 sm:px-10 ms:px-10 mb-25">
            ...
            <div className="flex justify-center py-20 border-y border-solid border-sectionbraek fixed bottom-0 w-full bg-white z-[3]">
